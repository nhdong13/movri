...
class AnnouncementBar extends Component {
  ...
  handleFormOnChange(e) {
    let announcementBarSetting = {...this.props.object} || {}
    let value = e.target.value
    if (e.target.type === 'checkbox') {
      value = e.target.checked
    }
    announcementBarSetting[e.target.name] = value
    this.props.handleChangeAnnouncementBarSetting(announcementBarSetting)
  }
  ...
  handleTextColorChangeComplete(color) {
    let announcementBarSetting = {...this.props.object} || {}
    announcementBarSetting['text_color'] = color.hex
    this.props.handleChangeAnnouncementBarSetting(announcementBarSetting)
  }

  handleBgColorChangeComplete(color) {
    let announcementBarSetting = {...this.props.object} || {}
    announcementBarSetting['background_color'] = color.hex
    this.props.handleChangeAnnouncementBarSetting(announcementBarSetting)
  }
      ...
      <div>
        ...
        <div className='checkbox-control'>
          <input
            ...
            name='enabled'
            type='checkbox'
            checked={announcementBarSetting['enabled'] || false}
            onChange={this.handleFormOnChange}
            id='announcement_enabled' />
          <label className='checkbox-label' htmlFor='announcement_enabled'>Show tag line</label>
        </div>
        <div className='checkbox-control'>
          <input
            className='checkbox-input'
            ...
            type='checkbox'
            checked={announcementBarSetting['home_only_enabled'] || false}
            onChange={this.handleFormOnChange}
            id='announcement_home_only_enabled' />
          <label className='checkbox-label' htmlFor='announcement_home_only_enabled'>Show on home page only</label>
        </div>
        <div className='form-control'>
          <label className='font-weight-regular'>Text</label>
          <SimpleCkeditor 
            name='text'
            value={announcementBarSetting['text'] || ''}
        ...
        <div className='form-control'>
          ...
          <input
            name='link'
            type='text'
            value={announcementBarSetting['link'] || ''}
            onChange={this.handleFormOnChange}/>
        </div>
        <div>
          <button
            type='button'
            className='color-box' 
            style={{backgroundColor: announcementBarSetting['text_color'] || '#000000'}} 
          ...
          {this.state.textColorEnabled && 
            <div>
              ...
              <SketchPicker
                color={announcementBarSetting['text_color'] || '#000000'}
        ...
        <div>
          <button
            type='button'
            className='color-box' 
            style={{backgroundColor: announcementBarSetting['background_color'] || '#000000'}} 
          ...
          {this.state.bgColorEnabled && 
            <div>
              <div style={ styles.cover } onClick={ this.handleClose }/>
              <SketchPicker
                color={announcementBarSetting['background_color'] || '#000000'}
