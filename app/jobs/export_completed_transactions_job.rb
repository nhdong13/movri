...
class ExportCompletedTransactionsJob < Struct.new(:current_user_id, :community_id, :export_task_id)
  ...
  def perform
    ...
    conversations = Transaction.includes(:shipper, :booking, :shipping_address, :billing_address, :custom_items, transaction_items: :listing).complete.order(order_number: :desc)
    csv_rows = []
  ...
  class << self
    def generate_csv_for(yielder, transactions)
      ...
      transactions.each do |transaction|
        @calculate_money_service = TransactionMoneyCalculation.new(transaction, {}, nil, true, false)
